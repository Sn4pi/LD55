...
if (abilityState == talisman.aim && image_index == clamp(image_index, 2, animation.throwImg[0])) {
	...
	var _chargeMax = 20;
	var _img = 0;
	_img = floor((1.13 - time_source_get_time_remaining(chargeTimer) / time_source_get_period(chargeTimer)) * _chargeMax);
	if (_img == _chargeMax) {
...
//TALISMAN Bar
	...
	var _chargeMax = 15;
	var _img = 0;
	_img = floor((1.13 - time_source_get_time_remaining(talisReady) / time_source_get_period(talisReady)) * _chargeMax);
	if (_img == _chargeMax) {
